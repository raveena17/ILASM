...
function createStageElement()
{
    ...
    btn.innerHTML = '<img src="/static/css/images/icon_deletelink.gif" />';    // var btn= document.images.namedItem("myImg").src;
    btn.appendChild(t);
    ...
    // newRow.childNodes[0].style.width = "120%"
    newRow.childNodes[0].appendChild(StageId);
    
    stagePlanned = 'milestone-'+ formNo + '-planned_days';
    planned = addElement("input", "number", "id_"+stagePlanned, stagePlanned);
    ...
    percent = addElement("input", "number", "id_"+stagePercent, stagePercent);
...
function createSpecificDatesElement()
{
    ...
    Table.appendChild(newRow);
    FormNoElement.value = parseInt(formNo) + 1;
    console.log(FormNoElement.value);
    DateTimeShortcuts.addCalendar(nameDate);
}
...
$(document).ready(function(){    
    ...
    for(var each=0; each<parseFloat("100"); each++){
    ...
    for(var each=0; each<parseFloat("100"); each++){
        $('#id_milestone-'+each+'-from_date').attr('class', 'vDateField');
        convertDate('id_milestone-'+each+'-from_date');

        };

    for(var each=0; each<parseFloat("100"); each++){
        $('#id_timebased-'+each+'-from_date').attr('class', 'vDateField');
        convertDate('id_timebased-'+each+'-from_date');
        $('#id_timebased-'+each+'-to_date').attr('class', 'vDateField');
        convertDate('id_timebased-'+each+'-to_date');
    ...
    $('#DeleteAllTimeBasedDetails').click(function(){
            if (window.confirm('Are you sure you want to delete all time-based dates details?'))
            {
                for(var each=0; each<parseFloat($('#id_timebased-TOTAL_FORMS').val()); each++){
                    $('#id_timebased-'+each+'-DELETE').attr('checked', true);
            };
            $('#redirectionUrl').val('/project/update/?ids='+$('#id_id').val());
            changeFormsetDateFormat();
            $("form:first").submit();
