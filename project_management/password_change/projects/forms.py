...
class ProjectBillingDetailForm(forms.ModelForm):
    ...
    from_date = forms.DateField(input_formats=[DATE_INPUT_FORMAT, '%Y-%m-%d'],
                                        required=False, widget=forms.TextInput(attrs=DATE_FIELD_ATTR))
    to_date = forms.DateField(input_formats=[DATE_INPUT_FORMAT, '%Y-%m-%d'],
                                        required=False, widget=forms.TextInput(attrs=DATE_FIELD_ATTR))
    ...
    def save(self, commit=True):
        instance = super(ProjectBillingDetailForm, self).save(commit=False)
        # import pdb;pdb.set_trace()
        if commit:
            instance.save()
        return instance
