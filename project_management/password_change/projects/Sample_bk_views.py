...
def project_initiation(request, msg=''):
    ...
    if request.method == 'POST':
        ...
        specific_dates_form = SpecificDatesFormSet(request.POST, queryset=get_milestone(
...
def manage_project_initiation_request(request):
    ...
    MilestoneFormSet = init_milestone_formset(
        ('name', 'percentage'), True, extra_form(
    ...
    if request.method == 'POST':
        ...
        if request_form.is_valid() and schedules_form.is_valid():
            ...
            # ex_appr = request.POST.get('ex_appr','1')
            ...
            pjtid = request_form.save(
                ...
                ex_approval=ex_appr)
            # pjtid=request_form.save(schedules = schedules, is_approved = is_approved, ex_approval = True)
            ...
            if request.user.has_perm(
                ...
                delivery_centre = str(
                    request_form.cleaned_data['delivery_centre'].name)
                ...
                email_message += bus_unit + "</td></tr><tr><td></br>Delivery Centre</td><td>:</td><td>" + \
                    delivery_centre + "</td></tr><tr><td></br>Project Name</td><td>:</td><td>" + \
                    project_name + "</td></tr>"
                ...
                try:
                    ...
                    if rejected_reason == '':
                        if str(ex_appr) == '1':
                            alert_generate(
                                request, 'Project', 'alertdataconfig13', id, email_to_list)
                        else:
                            alert_generate(
                                request, 'Project', 'alertdataconfig14', id, email_to_list)
                        if is_approved:
...
#                email_message = template.render(Context(data))
                ...
                mail_content += bus_unit + "</td></tr><tr><td>Delivery Centre</td><td>:</td><td>" + \
                    delivery_centre + "</td></tr><tr><td>Project Name</td><td>:</td><td>" + \
                    project_name + "</td></tr>"
    ...
    return render(request, 'project_initiation_request.html',
                  {'request_form': request_form, 'schedules_form': schedules_form, 'msg': actionMsg[msg], 'is_approved': is_approved,
                   ...
                   'milestone_form': milestone_form, 'user': request.user, 'approval_type': project_category,
