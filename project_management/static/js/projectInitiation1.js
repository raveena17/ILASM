...
function getclients(){
   ...
   $.getJSON("../getclients/?customer="+getdata, 
    function(json){
    document.getElementById('id_customer_contact').options.length =0;
        if (json.length > 0){
            for (j =json.length-1 ;j >=0;j--)
            {
             if ((/Firefox[\/\s](\d+\.\d+)/.test(navigator.userAgent))||(navigator.appName == "Microsoft Internet Explorer" )) {
               newLi = $('<option selected value='+json[j].id+'>'+json[j].name+'</option>');
                $("#id_customer_contact").prepend(newLi);
               }
              else {
        ...
        $("#id_customer_contact").prepend($('<option value="">---------</option>'));
        }
        else{
...
// function SetFocus () {
//     alert();
//     var input = document.getElementById ("id_project_name");
//     input.focus ();
// }
...
$(document).ready(function(){    
    ...
    convertDate('id_timebased-0-end_date');
     ...
     // SetFocus()
     // console.log("hello" + $('#id_project_name').focus()); 
    ...
    $('input#id_project_name').slugify('input#id_short_name');
    ...
    if ($("#id_approval_type_0").is(":checked")) {
