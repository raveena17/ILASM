...
def fetch_next_job_sp():
    ...
    try:
        ...
        cursor.callproc('fetchNextJob_SP')
...
def timesheet_alert(request):
    ...
    fetch_list_data = fetch_next_job_sp()

    for data in fetch_list_data:
        data = data[1]
        ...
        if not Holiday.objects.filter(Q(holdate=today_date) &
            ...
            # import pdb;pdb.set_trace()
            emp_in_leave =[]
            emp_list = LeaveRequests.objects.filter(Q(leave_from__lte=today_date,leave_to__gte=today_date)).exclude(approval_status='Cancelled').values_list('empid', flat=True)
            userlist = UserProfile.objects.filter(user_id__in=emp_list, business_unit_code__in=vorg).values_list('user', flat=True)
            user_name = User.objects.filter(id__in=userlist)

            for name in user_name:
                emp_in_leave.append(name.username)

            emailomission = Emailomission.objects.all().values('adsslogin')
            emp_id = User.objects.filter(is_active=True).exclude(
                username__in=emp_in_leave).exclude(username__in=emailomission).values_list('id', flat=True)
            userprofile_list = UserProfile.objects.filter(user_id__in=emp_id, business_unit_code__in=vorg).values_list('user', flat=True)
            user = User.objects.filter(id__in=userprofile_list)
...
#         emp_in_leave =[]
#         emp_list = LeaveRequests.objects.filter(Q(leave_from__lte=today_date,leave_to__gte=today_date)).exclude(approval_status='Cancelled').values_list('empid', flat=True)
#         user_name = User.objects.filter(id__in=emp_list)
#         for name in user_name:
#             emp_in_leave.append(name.username)
