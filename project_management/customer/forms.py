...
class CustomerContactProfileForm(forms.ModelForm):
    ...
    business_unit = forms.ModelChoiceField(
        queryset=BusinessUnit.objects.filter(
            type__name='Customer'), label='Customer')
...
#            self.fields['business_unit'].initial = customer_profile.business_unit.all()[0].pk
    ...
    def save(self, address, contact, commit=True):
        contact_profile = super(
            CustomerContactProfileForm,
            self).save(
            commit=False)
        ...
        if commit:
            ...
            contact_profile.business_unit.add(
                self.cleaned_data['business_unit'])
...
class ClientForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super(self.__class__, self).__init__(*args, **kwargs)
        self.fields['type'].queryset = BusinessUnitType.objects.filter(
            name='Customer')
        self.fields['related_to'].queryset = BusinessUnit.objects.exclude(
            type__name='Customer')
    ...
    class Meta:
        model = BusinessUnit
        exclude = ('cancel', 'address', 'customer_code', 'is_active','unit_code')
